...
const int N=1e5+5;
int mod;
int tree[4*N],d[N],lazy[4*N],mul[4*N]={1};
...
void updatetimes(int l,int r,int c,int s,int t,int p) {
    // 收敛到可表区间,进行修改，回溯，剪枝
    if (s>=l&&t<=r) {
        // 更新树对应值，打标记
        tree[p]*=c;
    ...
    if (lazy[p]) {
        // 有标记时
        tree[p*2]*=mul[p],mul[p*2]*=mul[p];
        tree[p*2+1]*=mul[p],mul[p*2+1]*=mul[p];
        mul[p]=0;
    }
    if (l<=mid) {
        updatetimes(l,r,c,s,mid,p*2);
    }
    if (r>mid) {
        updatetimes(l,r,c,mid+1,t,p*2+1);
    }
    tree[p]=tree[p*2]+tree[p*2+1];// 递归将更新返回父节点
}
